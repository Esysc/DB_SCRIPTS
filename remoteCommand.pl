...
my $ret = 0;
my $list;
my $REST_SERVER = "http://spmgt.my.comp.ltd";
sub getIPAddress() {
...
# get script name and script content here
		my $uriGetScript = $REST_SERVER.'/SPOT/provisioning/api/provisioningscripts/'.$f->{scriptid};
		my $req_script_name = HTTP::Request->new( 'GET', $uriGetScript);
		$req_script_name->authorization_basic('admin' => '2elleChap4');
...
#build the string
	...
	$req->authorization_basic('admin' => '2elleChap4');
