...
##EXAMPLE##
##EXAMPLE##    ${__SCRIPTNAME} -D create_dump=/var/tmp/debug ./${__SCRIPTNAME}
##EXAMPLE##
#
# Note: The escape character in the command below is only for the usage of scriptt.sh with the "-X" parameter!
#
##EXAMPLE## # use logger instead of echo to print the messages
##EXAMPLE##
##EXAMPLE##    LOGMSG_FUNCTION=\"logger -s -p user.info ${__SCRIPTNAME}\"  ./${__SCRIPTNAME}
##EXAMPLE##
##EXAMPLE##
...
#  use -D setvar:varname=varvlaue to init the envrionment, print the internal variables and end the script
...
# (to disable only parts of the house keeping use one of the variables:
# __NO_EXIT_ROUTINES  __NO_TEMPFILES_DELETE __NO_TEMPMOUNTS_UMOUNT  __NO_TEMPDIR_DELETE  __NO_FINISH_ROUTINES __CLEANUP_ON_ERROR
# )
...
# Use -D tracefunc=x to trace the function GetSeconds
 ...
 # Use -D tracefunc=x to trace the functions GetSeconds and CreateTemporaryFiles, and SetEnvironment
 ...
 # use -D debugcode=x to enable traceing (set -x) for the function GetSeconds only
 ...
 Use the parameter -D debugcode=x with care! And do NOT use code that writes to STDOUT - all output must go to another file handle, e.g STDERR or /dev/tty!
...
##V#     two new aliase are defined: __ingoretraps and __unsettraps
...
####
typeset -r __SCRIPT_TEMPLATE_VERSION="2.1.0.10 16.11.2015"
####
...
##V#      added the variables __SETOPTS and __TRACE_ACTIVE
...
# create a temporary mount point or directory
    ...
    mkdir "${TMPDIR}" && __LIST_OF_TMP_DIRS="${__LIST_OF_TMP_DIRS} ${TMPDIR}" || die 10 "Error creating the directory \"${TMPDIR}\" "

# create a temporary file

    touch "${TMPFILE}" && __LIST_OF_TMP_FILES="${__LIST_OF_TMP_FILES} ${TMPFILE}" || die 10 "Error creating the file \"${TMPFILE}\" "
...
#
  if [ ${__PRINT_INTERNAL_VARIABLES}x = ${__TRUE}x ] ; then
